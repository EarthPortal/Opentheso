INSERT INTO languages_iso639 (iso639_1, iso639_2, english_name, french_name, code_pays)
VALUES
    ('hi-Latin', 'hi-latin', 'Hindi (Latin)', 'Hindi (Latin)', 'in'),
    ('mr-Latin', 'mr-latin', 'Marathi (Latin)', 'Marathi (Latin)', 'in')
    ON CONFLICT (iso639_1) DO NOTHING;
...
FOR traduction_rec IN SELECT * FROM opentheso_get_preflabel_traductions(idtheso, idconcept, idlang)
                                        LOOP
    preflab = preflab || traduction_rec.term_id || sous_seperateur || traduction_rec.term_lexical_value || sous_seperateur || traduction_rec.term_lang || sous_seperateur || traduction_rec.unique_id || sous_seperateur || COALESCE(traduction_rec.flag_code, '') || seperateur;
END LOOP;
...
RETURNS TABLE(
    ...
    term_lexical_value character varying,
    term_lang character varying,
    unique_id integer,
...
SELECT
    ...
    term.lang,
    term.id,
    languages_iso639.code_pays
FROM term
         JOIN preferred_term
              ON term.id_term = preferred_term.id_term
                  AND term.id_thesaurus = preferred_term.id_thesaurus
         LEFT JOIN languages_iso639
                   ON term.lang = languages_iso639.iso639_1
WHERE term.id_thesaurus = id_theso
